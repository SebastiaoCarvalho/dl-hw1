...
the hidden units is either -1 or 1, and computing the AND we can verify if they respect both the lower bound and upper bound, and thus
...
$, and we perturb it to make it $X' = \begin{bmatrix}
    1 + \epsilon
\end{bmatrix}
$, with $\epsilon \in \mathbb{R}$ and $\epsilon > 0$. Since $\epsilon$ is a really small number, we have that $h(X) = 1$ and $h(X') = -1$, 
and thus the current network is not robust to infinitesimal perturbation of the inputs.
...
(\epsilon < 1)$. This way we can rewrite the inequations as:
